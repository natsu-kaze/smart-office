package com.natsukaze.smartoffice.attendance.service;
...
import com.natsukaze.smartoffice.attendance.entity.AttendanceRule;
import com.natsukaze.smartoffice.attendance.entity.AttendanceSummary;
import com.natsukaze.smartoffice.attendance.mapper.AttendanceRecordMapper;
import com.natsukaze.smartoffice.attendance.mapper.AttendanceRuleMapper;
import com.natsukaze.smartoffice.attendance.mapper.AttendanceSummaryMapper;
...
public class AttendanceService {
    ...
    public PageResult<AttendanceRecordVO> pageRecords(AttendanceRecordQuery query) {
        ...
        if (query.getDepartmentId() != null) {
            userIds = employeeMapper.selectList(new LambdaQueryWrapper<OrgEmployee>()
                            .eq(OrgEmployee::getDepartmentId, query.getDepartmentId()))
                    .stream()
    ...
    private AttendanceRecordVO toRecordVO(AttendanceRecord record) {
        ...
        OrgEmployee employee = employeeMapper.selectOne(new LambdaQueryWrapper<OrgEmployee>()
                .eq(OrgEmployee::getUserId, record.getUserId())
                .last("LIMIT 1"));
        OrgDepartment department = employee == null ? null : departmentMapper.selectById(employee.getDepartmentId());
        return AttendanceRecordVO.builder()
                .id(record.getId())
                .userId(record.getUserId())
                .realName(user == null ? null : user.getRealName())
                .departmentId(employee == null ? null : employee.getDepartmentId())
    ...
    private AttendanceSummaryVO toSummaryVO(AttendanceSummary summary) {
        SysUser user = userMapper.selectById(summary.getUserId());
        return AttendanceSummaryVO.builder()
