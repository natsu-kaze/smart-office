...
* approval-service：审批草稿、提交、通过、驳回、撤回、关闭、审批节点和记录。
  ...
  * 请假审批通过后，会把请假日期同步到考勤记录，避免已批准请假仍被统计为缺勤。
...
* smart-office-web：登录、工作台、用户、组织、审批、消息、文件、考勤、制度文档检索、审批详情、考勤记录、角色权限和个人中心页面。
* OpenFeign：核心内部调用已接入 Sentinel fallbackFactory，并通过 Nacos 共享 Sentinel 配置管理降级规则。
...
2. 补公告发布、草稿编辑、头像上传、请假同步考勤、审批超时扫描、制度上传解析的网关冒烟脚本。
